...
public class BeanUsuarios{
    ...
    public void insertar() {
        ...
        if (nombreUsuario == null || nombreUsuario.trim().isEmpty() || rol == null || rol.trim().isEmpty() || "Selecciona un rol".equals(rol)
                || contrasenia == null || contrasenia.trim().isEmpty()) {
            context.addMessage(null, new FacesMessage("Datos", "Debe llenar todos los campos"));
        } else {
            if (CRUDs.CRUDUsuario.insert(nombreUsuario, rol, contrasenia)) {
                this.limpiar();
                context.addMessage(null, new FacesMessage("Exito", "Registro Ingresado"));
                setListUsuarios(CRUDs.CRUDUsuario.universo());
            } else {
                context.addMessage(null, new FacesMessage("Error", "Información no se guardó"));
            }
        }
    }
